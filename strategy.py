...
def m15_gold_strategy(data: pd.DataFrame) -> pd.DataFrame:
    ...
    macd = ta.macd(data['close'], fast=7, slow=17, signal=9)
    data['macd_line'] = macd['MACD_7_17_9']
    data['macd_signal'] = macd['MACDs_7_17_9']
    
    data['lsma_stddev'] = data['close'].rolling(window=10).std()
    ...
    data['lsma_upper_band'] = data['lsma'] + 1.5*data['lsma_stddev']
    data['lsma_lower_band'] = data['lsma'] -1.5* data['lsma_stddev']

    # Calculate stochastic oscillator
    stochastic = ta.stoch(data['high'], data['low'], data['close'], k=5, d=3)
    data['stoch_k'] = stochastic['STOCHk_5_3_3']
    data['stoch_d'] = stochastic['STOCHd_5_3_3']
    ...
    # Generate signals
    data['is_buy2'] = (
        ((data['prev_fixed_support_trendline'] < data['prev_4fixed_support_trendline'].shift(1))&
         (data['HSpan_A']>data['HSpan_B'])
         )
    ...
    data['is_sell2'] = (
        ((data['prev_fixed_resistance_trendline'] > data['prev_4fixed_resistance_trendline'].shift(1))&
         (data['HSpan_A']<data['HSpan_B']))
    )
